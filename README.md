...
To solve this, `SDLPointer<SDLType>` creates a genericized interface to wrap the `OpaquePointer` instances. Being a `SDLType` adopter means describing the pointer's intent as an object (`SDLWindow`, `SDLRenderer`, `SDLTexture`, etc...), and implementing a function which invokes the corresponding `SDL2` API associated with to freeing said pointer.  
